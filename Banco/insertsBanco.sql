...
SELECT monthly_amount, extra_income, date_revenue FROM revenue_tbl WHERE FK_cod_user_revenue = 2;
...
SELECT user_name, monthly_amount, extra_income, SUM(amount_expenditure), revenue_id FROM users_tbl INNER JOIN revenue_tbl ON users_tbl.user_id = revenue_tbl.FK_cod_user_revenue INNER JOIN expenditure_tbl ON users_tbl.user_id = expenditure_tbl.FK_cod_user_expenditure WHERE user_id = 2;
...
SELECT user_name, monthly_amount, extra_income, SUM(amount_expenditure) FROM users_tbl INNER JOIN revenue_tbl ON users_tbl.user_id = revenue_tbl.FK_cod_user_revenue INNER JOIN expenditure_tbl ON users_tbl.user_id = expenditure_tbl.FK_cod_user_expenditure;
